...
static int snd_bbfpro_gain_update(struct usb_mixer_interface *mixer, u8 channel, int gain)
{
    ...
    if (channel >= SND_BBFPRO_GAIN_CHANNELS)
        return -EINVAL;
    ...
    if (channel < 2) {
        if (gain < SND_BBFPRO_GAIN_MIN1)
            gain = SND_BBFPRO_GAIN_MIN1;
        else if (gain > SND_BBFPRO_GAIN_MAX1)
            gain = SND_BBFPRO_GAIN_MAX1;
        
        // Handle the quirk for 0x01 - 0x09 gap
        if (gain > 0 && gain < 10)
            gain = 10;
    } else {
        if (gain < SND_BBFPRO_GAIN_MIN2)
            gain = SND_BBFPRO_GAIN_MIN2;
        else if (gain > SND_BBFPRO_GAIN_MAX2)
            gain = SND_BBFPRO_GAIN_MAX2;
    }
...
static int snd_bbfpro_gain_info(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_info *uinfo)
{
    ...
    if (channel < 2) {
        uinfo->value.integer.max = SND_BBFPRO_GAIN_MAX1;
        uinfo->value.integer.step = 1;
    } else {
        uinfo->value.integer.max = SND_BBFPRO_GAIN_MAX2;
        uinfo->value.integer.step = 1;  // ALSA will interpret this as 0.5 dB steps
    }
    
    return 0;
}

static int snd_bbfpro_gain_get(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
...
static int snd_bbfpro_gain_put(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
